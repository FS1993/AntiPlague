...
public class KrajFrameLatwy extends JFrame {
	
	private Component KrajFrameLatwy;
	int kryterium1 = 0,
		kryterium2 = 0,
		kryterium3 = 0;
		
	
	public KrajFrameLatwy(String nazwa, KrajButton kraj, List<KrajButton> kraje, Latwy poziomLatwy, JTextField zarazeniTextField, JTextField punktyTextField) {
		...
		Thread glowny = new Thread(()->{
			while(poziomLatwy.zarazeniSwiata < poziomLatwy.populacjaSwiata && poziomLatwy.zarazeniSwiata > 0 ) {
		...
		Thread leki = new Thread(()->{
			while(poziomLatwy.zarazeniSwiata <= poziomLatwy.populacjaSwiata && poziomLatwy.zarazeniSwiata >= 0 ) {
				poziomLatwy.punkty+= 500;
				punktyTextField.setText("Punkty: "+poziomLatwy.punkty);
				if(poziomLatwy.zarazeniSwiata > 0)
					poziomLatwy.zarazeniSwiata-=500;
				if(kraj.zarazeni > 0)
					kraj.zarazeni-=500;
				zarazeniTextField.setText("Zara¿eni: " + poziomLatwy.zarazeniSwiata);
				zarazeniKrajuTextField.setText("Zara¿eni: " + kraj.zarazeni);
				try {
					Thread.sleep(10000);
		...
		Thread szczepionka = new Thread(()->{
			while(poziomLatwy.zarazeniSwiata <= poziomLatwy.populacjaSwiata && poziomLatwy.zarazeniSwiata >= 0 ) {
				poziomLatwy.punkty+= 1000;
				punktyTextField.setText("Punkty: "+poziomLatwy.punkty);
				if(poziomLatwy.zarazeniSwiata > 0)
					poziomLatwy.zarazeniSwiata-=1000;
				if(kraj.zarazeni > 0)
					kraj.zarazeni-=1000;
				zarazeniTextField.setText("Zara¿eni: " + poziomLatwy.zarazeniSwiata);
				zarazeniKrajuTextField.setText("Zara¿eni: " + kraj.zarazeni);
		...
		ulepszenie2.addActionListener(new ActionListener(){
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if (poziomLatwy.punkty >= 3000) {
					poziomLatwy.punkty-=3000;
					leki.start();
					ulepszenie2.setEnabled(false);
				}
				else {
					JOptionPane.showMessageDialog(KrajFrameLatwy, "Brakuje ci punktów, ¿eby kupiæ to ulepszenie.", "Brak punktów", JOptionPane.WARNING_MESSAGE);
				}
			}
 		});
		...
		ulepszenie3.addActionListener(new ActionListener(){
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if (poziomLatwy.punkty >= 5000) {
					poziomLatwy.punkty-=5000;
					szczepionka.start();
		...
		ulepszenie4.addActionListener(new ActionListener(){
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if (poziomLatwy.punkty >= 9000) {
					poziomLatwy.punkty-=9000;
					punktyTextField.setText("Punkty: "+poziomLatwy.punkty);
					wirusWew.stop();
		...
		ulepszenie5.addActionListener(new ActionListener(){
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if (poziomLatwy.punkty >= 10000) {
					poziomLatwy.punkty-=10000;
					punktyTextField.setText("Punkty: "+poziomLatwy.punkty);
