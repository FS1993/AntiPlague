...
public class KrajFrameTrudny extends JFrame {
	
	private Component KrajFrameTrudny;
	int kryterium1 = 0,
		kryterium2 = 0,
		kryterium3 = 0;
		
	
	public KrajFrameTrudny(String nazwa, KrajButton kraj, List<KrajButton> kraje, Trudny poziomTrudny, JTextField zarazeniTextField, JTextField punktyTextField) {
		...
		Thread glowny = new Thread(()->{
			while(poziomTrudny.zarazeniSwiata < poziomTrudny.populacjaSwiata && poziomTrudny.zarazeniSwiata > 0 ) {
		...
		Thread leki = new Thread(()->{
			while(poziomTrudny.zarazeniSwiata <= poziomTrudny.populacjaSwiata && poziomTrudny.zarazeniSwiata >= 0 ) {
				poziomTrudny.punkty+= 500;
				punktyTextField.setText("Punkty: "+poziomTrudny.punkty);
				if(poziomTrudny.zarazeniSwiata > 0)
					poziomTrudny.zarazeniSwiata-=500;
				if(kraj.zarazeni > 0)
					kraj.zarazeni-=500;
				zarazeniTextField.setText("Zara¿eni: " + poziomTrudny.zarazeniSwiata);
				zarazeniKrajuTextField.setText("Zara¿eni: " + kraj.zarazeni);
				try {
					Thread.sleep(10000);
		...
		Thread szczepionka = new Thread(()->{
			while(poziomTrudny.zarazeniSwiata <= poziomTrudny.populacjaSwiata && poziomTrudny.zarazeniSwiata >= 0 ) {
				poziomTrudny.punkty+= 1000;
				punktyTextField.setText("Punkty: "+poziomTrudny.punkty);
				if(poziomTrudny.zarazeniSwiata > 0)
					poziomTrudny.zarazeniSwiata-=1000;
				if(kraj.zarazeni > 0)
					kraj.zarazeni-=1000;
				zarazeniTextField.setText("Zara¿eni: " + poziomTrudny.zarazeniSwiata);
				zarazeniKrajuTextField.setText("Zara¿eni: " + kraj.zarazeni);
		...
		ulepszenie2.addActionListener(new ActionListener(){
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if (poziomTrudny.punkty >= 3000) {
					poziomTrudny.punkty-=3000;
					leki.start();
					ulepszenie2.setEnabled(false);
				}
				else {
					JOptionPane.showMessageDialog(KrajFrameTrudny, "Brakuje ci punktów, ¿eby kupiæ to ulepszenie.", "Brak punktów", JOptionPane.WARNING_MESSAGE);
				}
			}
 		});
		...
		ulepszenie3.addActionListener(new ActionListener(){
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if (poziomTrudny.punkty >= 5000) {
					poziomTrudny.punkty-=5000;
					szczepionka.start();
		...
		ulepszenie4.addActionListener(new ActionListener(){
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if (poziomTrudny.punkty >= 9000) {
					poziomTrudny.punkty-=9000;
					punktyTextField.setText("Punkty: "+poziomTrudny.punkty);
					wirusWew.stop();
		...
		ulepszenie5.addActionListener(new ActionListener(){
			...
			@Override
			public void actionPerformed(ActionEvent e) {
				if (poziomTrudny.punkty >= 10000) {
					poziomTrudny.punkty-=10000;
					punktyTextField.setText("Punkty: "+poziomTrudny.punkty);
